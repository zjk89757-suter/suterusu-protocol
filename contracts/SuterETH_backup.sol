...
contract SuterETH_backup {
    ...
    /* 
       The # of wei that constitute one unit.
       Balances, funds, burns, and transfers are all interpreted in terms of unit, rather than wei. 
    */
    uint256 public constant unit = 1e16; 
    ...
    */
    ...
    function toUnit(uint256 rawAmount) internal view returns (uint256) {
        require(msg.value % unit == 0, "Amount must be multiple of 1e16 wei (0.01 ETH).");
        uint256 amount = rawAmount / unit;
        require(0 <= amount && amount <= MAX, "Amount out of range."); // uint, so other way not necessary?
        return amount;
    }

    // Debug only. Should be removed before delivery.
    function fundPlain() public payable {
        plainBalances[msg.sender] += msg.value;
        require(address(this).balance <= MAX, "Fund pushes contract past maximum value.");
    }

    // Debug only. Should be removed before delivery.
    function burnPlain(uint256 bTransfer) public {
        require(plainBalances[msg.sender] >= bTransfer, "Withdraw more fund than available!");
        msg.sender.transfer(plainBalances[msg.sender]);
    }

    // Debug only. Should be removed before delivery.
    function getBalancePlain(address account) public view returns (uint256) {
        return plainBalances[account];
    }
    ...
    */
    ...
    function fund(Utils.G1Point memory y) public payable {
        bytes32 yHash = keccak256(abi.encode(y));
        require(registered(yHash), "Account not yet registered.");
        rollOver(yHash);

        uint256 amount = toUnit(msg.value);
    ...
    function burn(Utils.G1Point memory y, uint256 amount, Utils.G1Point memory u, bytes memory proof) public {
        bytes32 yHash = keccak256(abi.encode(y));
        require(registered(yHash), "Account not yet registered.");
        rollOver(yHash);
